  ...
  $.ajax({
    ...
    success(response) {
      ...
      if (response.status === 'OK') {        
        ...
        $('#notification').fadeIn()
        $('#notification').html(`<b>Goal!</b> ${response.player} scored a goal for ${response.team.name}.`)
  ...
  $.ajax({
    ...
    success(response) {
      ...
      if (response.status === 'OK') {
        ...
        notif.className = 'alert alert-success'
        $('#notification').fadeIn()
        $('#notification').html(`<b>GAME ENDED ON ${new Date(response.time).toLocaleString()}!</b> Snitch was caught by ${response.player}.`)
      } else {
        notif.className = ''
        notif.className = 'alert alert-danger'
        $('#notification').fadeIn()
  ...
  $.ajax({
    ...
    success(response) {
      ...
      if (response.status === 'OK') {
        ...
        notif.className = 'alert alert-info'
        $('#notification').fadeIn()
        $('#notification').html(`<b>Snitch has appeared</b> on <br /> ${new Date(response.time).toLocaleString()}`)
        $('#notification').delay(2000).slideUp()
      } else {
        notif.className = ''
        notif.className = 'alert alert-danger'
